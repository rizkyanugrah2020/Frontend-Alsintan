        ...
        <q-tr :props="props">
          ...
          <q-td key="aksi" :props="props">
            <div class="justify-center q-gutter-x-xs">
              <q-btn label="Detail" unelevated color="dark" :to="{ name: detailRoute, params: {id: props.row.id}}"/>
              <q-btn label="Tolak" :disable="props.row.idstatus >= statusOn" color="negative" unelevated @click="openDialog(props.row.id, statusTolak)"/>
              <q-btn label="Konfirmasi" :disable="props.row.idstatus >= statusOn" unelevated @click="openDialog(props.row.id, statusKonfirmasi)" color="warning"/>
            </div>
          </q-td>
        </q-tr>
      </template>
  ...
  methods: {
    ...
    openDialog (id, status) {
      this.$q.dialog({
        ...
        this.$showLoading()
        this.$axios.put(`asuransi/acc-jasindo/${id}`, body, this.$createToken())
          .finally(() => this.$hide())
          .then(res => {
