        ...
        <q-tr :props="props">
          <q-td key="aksi" :props="props">
            <div class="q-gutter-sm">
              <q-btn v-if="this.$route.name === 'dataPengajuanAdmin'" label="Detail" :to="{ name: 'detailPengajuanAdmin', params: { id: props.row.id_master_beasiswa } }" no-caps color="amber" outline/>
              <q-btn v-else label="Detail" :to="{ name: 'detailPengajuanBeasiswa', params: { id: props.row.id_master_beasiswa } }" no-caps color="amber" outline/>
              <q-btn v-if="this.$route.name === 'dataPengajuanAdmin'" label="Hapus" @click="deleteRecomendation(props.row.id_master_beasiswa )" no-caps color="negative" outline/>
            </div>
          </q-td>
...
export default {
  ...
  methods: {
    getData () {
      this.$showLoading()
      this.$axios.get('beasiswa/getRecomendations')
        .finally(() => this.$hide())
        .then((res) => {
    ...
    deleteRecomendation (id) {
      this.$showLoading()
      this.$axios.delete(`beasiswa/deleteRecomendations/${id}`, this.$createToken())
        .finally(() => this.$hide())
        .then((res) => {
          if (res.data.status) {
