      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          <q-td key="aksi" :props="props">
            <q-btn label="Detail" :to="{ name: 'detailPengajuanInstitut', params: { id: props.row.id_master_beasiswa } }" no-caps color="amber" outline/>
          </q-td>
...
export default {
  ...
  methods: {
    getData () {
      this.$showLoading()
      this.$axios.get(`beasiswa/getRecomendationsbyUniv/${this.$getProfile().member.id_member}`)
        .finally(() => this.$hide())
        .then((res) => {
