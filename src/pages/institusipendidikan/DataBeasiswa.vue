<template>
  <q-page padding>
    <q-banner v-if="this.univ === null" inline-actions class="text-white bg-red q-mb-sm" style="border-radius: 5px">
      Anda Belum Mengisi form data diri, silahkan diisi
    </q-banner>
                ...
                <div class="row justify-between items-center">
                  ...
                  <div class="col-auto items-end">
                    <q-btn
                      ...
                      text-color="white"
                      label="Lihat Detail"
                      :to="{ name: 'detailbeasiswaInstitut', params: { id: props.row.id } }"
                    />
                  </div>
                </div>
              </q-card>
...
export default {
  ...
  methods: {
    getData () {
      this.$showLoading()
      this.$axios.get(`beasiswa/getBeasiswabyUniv/${this.univ.id}`, this.$createToken())
        .finally(() => this.$hide())
        .then(res => {
    ...
    getUniv () {
      ...
      this.$axios.get(`beasiswa/getUnivMember/${this.$getProfile().member.id_member}`, this.$createToken())
        .finally(() => this.$hide())
        .then(res => {
          this.univ = res.data.result
          this.form.id_institusi = this.univ.id
          this.getData()
        })
        .catch(() => this.$commonErrorNotif())
    },
