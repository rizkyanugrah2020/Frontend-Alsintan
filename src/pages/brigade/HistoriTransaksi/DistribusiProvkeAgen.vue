  ...
  <q-page padding>
    <q-card flat>
      <q-card-section class="text-center">
        <span class="text-h6">Riwayat Permohonan/ Transaksi Distribusi Ke Agen</span>
      </q-card-section>
      <q-separator/>
      <q-table
        :rows="rows3"
...
export default {
  ...
  methods: {
    getPengajuan () {
      this.$showLoading()
      this.$axios.get(`alsintan/transaksi/brigade/riwayat-pengajuan/${profile.member.id_member}/4`, this.$createToken())
        .finally(() => this.$hide())
        .then(res => {
          // console.log(res)
